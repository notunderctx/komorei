...
const Recommendations: React.FC<RecProps> = ({ info }) => {
  const [showAll, setShowAll] = useState(false);
  const RecInfo = Array.isArray(info?.nodes) ? info?.nodes : [];

  return (
    <CharactersContainer>
      {/* For larger devices */}
      <CharactersGrid>
        {RecInfo?.map((item: any, index: number) => (
          <CharacterCard key={index}>
            <a
              className="group relative hover:scale-105 hover:shadow-lg duration-300 ease-out"
              href={`/streaming/${item?.id}${item?.media?.type === "MANGA" ? "&t=MANGA" : ""}`}>
              <CharacterImage
                src={
            ...
            <CharacterName>{item?.media?.title?.romaji || ""}</CharacterName>
          </CharacterCard>
        ))}
      </CharactersGrid>
    </CharactersContainer>
